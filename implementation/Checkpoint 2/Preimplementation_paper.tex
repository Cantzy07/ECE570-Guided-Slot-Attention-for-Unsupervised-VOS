...
\textbf{3.	How to improve computation speed while also widening the receptive field for occupancy area prediction in self-driving vehicles}
...
\hspace{20pt} The main contribution of this paper is the guided slot generator and slot attention framework that allows for state of the art foreground and background separation.  The slot generator takes the embeddings for the target image feature and pushes it through a 1x1 convolutional layer then applies a softmax operation on the output. Afterwards this output undergoes a global weighted average pooling with the target feature embedded in the local extractor which yields the guided slot blocks that assist in the final mask generation. Utilizing the slot attention module introduced in the \textit{Object-Centric Learning with Slot Attention} [5] paper by Google Research, slot attention can extract object-centric representations that enable generalization for unseen compositions which make it ideal for unsupervised learning.
...
Initial implementation for the project is based off the architecture for \textit{Guided Slot Attention for Unsupervised Video Object Segmentation} [1] Paper. It explored the ResNet101 and MiT-b2 encoder backbones presented in the paper and chose the MiT-b2 encoder as the one to be used in the preliminary implementation. The goal of the pre-implementation was to familiarize myself with encoding a raw rgb image into a feature embedding, methodology between local and global feature extraction from the embedding, and conceptual understanding of the slot attention process. 

\textbf{Preliminary Implementation Efforts: }
The MiT-b2 encoder backbone was used to process a raw rgb input image and output encoded features from the given image. The hidden sizes for the features consist of [64, 128, 320, 512]. After this the input image was put through 3 different pathways for processing. The first path was the Slot Generator, which passed the encoded input image features through a 1x1 convolution then performed the pixel-wise softmax shown below, where $N_s$ is the number of slots (2 for foreground and background slots), and X is the input image:

\begin{align}
    M_{S(x,y)}^{i} &= \frac{e^{X_{S(x,y)}^{i}}}{\sum_{i=1}^{N_s} e^{X_{S(x,y)}^{i}}}
\end{align}

\hspace{20pt}After this, global weighted average pooling was performed with the target image feature embedded in the local extractor using the methodology below by taking the input feature tensor and multiplying it element-wise with the cluster masks:

\begin{figure}[h]
    \centering
    \includegraphics[width=0.25\textwidth]{Example Feature Map.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.25\textwidth]{Example Cluster Mask.png}
    \caption{Cluster Mask Example}
    \label{fig:image_label}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.25\textwidth]{Example Result.png}
    \caption{Element-Wise Multiplication}
    \label{fig:image_label}
\end{figure}

\hspace{20pt}Finally, the resulting matrix would get summed and divided it by the sum of the cluster mask. This would output several vectors each representing that specific cluster mask and its prominence in each channel of the encoded feature embedding. For the simplified example above, the resulting score would be 5.

\hspace{20pt}Secondly, the local extractor path also performs a 1x1 convolution on the target image feature then performs k-means on the features to develop sets of cluster masks. Then global weighted average pooling is performed on the embedded local feature and the cluster masks to create vectors for each cluster representing the prominence in each channel.

\hspace{20pt}Lastly, the global extractor path first has the encoded features from the MiT-b2 architecture pass through a feature pyramid network similar to the one laid out in the reference paper \textit{“Feature Pyramid Networks for Object Detection”} [7]. This consecutively dilates up the image size in order to prioritize broader features in the image. After performing this the encoded features undergo the same 1x1 convolution before having a channel-wise softmax operation shown below performed then doing a global weighted average pooling similar to the ones in the previous paths. Where H and W are the height and width of the image in pixels respectively, j is the number of channels, and X is the input image:

\begin{align}
    M_{G_{t}(x,y)}^{j} &= \frac{e^{X_{G_{t}(x,y)}^{j}}}{\sum_{x=1}^{H} \sum_{y=1}^{W} e^{X_{G_{t}(x,y)}^{j}}}
\end{align}

\hspace{20pt}\textbf{Next Steps for Implementation: }
the next steps to take for the implementation would be to develop the feature aggregation transformer as laid out in the original paper. This would involve KNN filtering, applying the algorithm to implement guided slots and slot attention, multilayer perceptrons, and softmax operations:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{FAT.png}
...
\textbf{Conclusion: }
This project proposes an implementation of a guided slot attention mechanism for video object segmentation in an unsupervised learning context. Additionally, it aims to explore various unsupervised learning algorithms in order to integrate them into the existing slot attention module laid out in the original paper: \textit{Guided Slot Attention for Unsupervised Video Object Segmentation}. By introducing object-centric slot attention through guided slots, background and foreground objects can be recognized and separated allowing for a more accurate masking of the most prominent object in a video object sequence.  Especially for crowded and complex foreground and background scenes, performing slot attention on slots in conjunction with extracted features from the given scene can yield accurate object segmentation that is beneficial for ongoing research and technological application in object detection, tracking, and segmentation.
...
\item Tafasca, S., Gupta, A., \& Odobez, J. (2023). Sharingan: A Transformer-based Architecture for Gaze Following. ArXiv. https://arxiv.org/abs/2310.00816

\item Agro, B., Sykora, Q., Casas, S., \& Urtasun, R. (2023). Implicit Occupancy Flow Fields for Perception and Prediction in Self-Driving. ArXiv. https://arxiv.org/abs/2308.01471
