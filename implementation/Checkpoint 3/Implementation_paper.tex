...
\twocolumn[
\icmltitle{Re-Implementing Guided Slot Attention for Video Object Segmentation}
...
\section*{\centering ECE57000: Artificial Intelligence Anonymous Submission}
...
The slot generator takes the local feature produced by the RGB encoder and passes it through a 1x1 convolutional layer in order to compress the channel size down to $N_s$ where $N_s$ is the number of slots (1 for foreground and 1 background slot), and X is the input image. After this a pixel-wise SoftMax is performed on the tensor before being pooled. 

\begin{align}
    M_{S(x,y)}^{i} &= \frac{e^{X_{S(x,y)}^{i}}}{\sum_{i=1}^{N_s} e^{X_{S(x,y)}^{i}}}
\end{align}\\

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{FAT.drawio.png}
    \caption{Feature Aggregation Transformer}
    \label{fig:image_label}
\end{figure}
...
\hspace{20pt} The decoder accepts the aggregated features, refined slots, and the third hidden layer of the original target image encoding. Cosine similarity is calculated between the aggregated features and the original encoding to produce a tensor $CM_a$, the original encoding and the foreground slot to produce $CM_{sf}$, and the original encoding and the background slot to produce $CM_{sb}$. These cosine similarities are then reshaped and concatenated channel-wise before being put into a softmax operation to produce a mask for the target image. Finally, the output mask it put into a refinement sequential block to improve accuracy to the target annotation mask.
\hspace{20pt} The cosine similarity performed is as shown below where $CM_a$ and $CM_{S_r}$ are the calculated cosine similarities, $X_L$ is the encoding from the original target image, $P_A^m$ is the aggregated feature tensor, and $P_{S_r}$ are the foreground and background slots with $i$ denoting the slot.

\begin{align}
\mathrm{CM}_A(x,y)
=
\left\{
  \frac{X_L(x,y) \cdot \mathbf{P}_A^m}
       {\|X_L(x,y)\|\;\|\mathbf{P}_A^m\|}
\right\}_{m}.
\end{align}
...
=
\left\{
  \frac{X_L(x,y) \cdot \mathbf{P}_{S_r}^i}
       {\|X_L(x,y)\|\;\|\mathbf{P}_{S_r}^i\|}
\right\}_{m}.
\end{align}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{segmentation mask test.png}
    \vspace{1em} % optional vertical space between images
    \includegraphics[width=\textwidth]{camel mask.png}
    \vspace{-4em} % optional vertical space between images
    \caption{\centering Segmentation Mask Testing}
    \label{fig:combined_images}
\end{figure*}

\section*{Experimental Results}

\hspace{20pt} Training was completed using the DAVIS-16 dataset, which contains 480p and 1080p images with associated annotation masks as the ground truth segmentation mask. The training file contained in the dataset give side-by-side images and annotation names for use in loss calculation and gradient backpropagation during training. For each iteration of training, one target image is selected and the next 5 images from the same image set are used as reference images for global extraction.

\hspace{20pt} Cross-Entropy loss was used to calculate loss between the output and ground-truth mask, and the Adam optimizer was used for stochastic optimization. The model was trained using Google Colab with the A100 hardware accelerator for 5 epochs, which took ~2.5 hours. Pytorch was used as the framework for the model implementation and training. 

\hspace{20pt} The purpose of this re-implementation was to display results using slots to separate the foreground and background, and thus does not use a separate path for optical flow like the original implementation doe,s and instead focuses on VOS on an image-by-image basis. The original implementation also utilized various other datasets such as the DUTS, Youtube-VOS, and FBMS datasets. For simplicity and proof of concept, only the DAVIS16 dataset was used due to having a robust amount of different scenes and annotated segmentation masks.

\hspace{20pt} The re-implementation can distinctly separate the foreground object and the background relatively accurately. The main issue with the predicted mask is the sparsity of the desired image mask. It can identify the general shape of the mask but struggles to bridge gaps to form a cohesive mask. Additionally, the model picks up other objects in the foreground due to the foreground objects having a higher relative scene prominence than background objects. This could be due to a multitude of reasons, such as the simplification of the feature aggregation transformer and slot attention transformer. Additionally, a global weighted average pooling was done on the local and global features before being used as input to the feature aggregation transformer which was skipped for the sake of simplicity in this re-implementation.

\textbf{Evaluative Metrics}

\hspace{20pt} After being trained the model yielded a test accuracy across the DAVIS-16 validation set of 86.3. 

The Jaccard Index is used to evaluate the overlap of segmentation mask pixels and was calculated by dividing the intersection of segmentation pixels similar to both the ground-truth mask and the output mask and the union of segmentation pixels that are in the ground-truth mask and/or the output mask as shown in the equation below.

\begin{equation}
\mathcal{J} = \frac{|A \cap B|}{|A \cup B|}
\end{equation}

The Boundary Index is used to assess the quality of boundary prediction by calculating the number of boundary pixels that fall within a tolerable distance. $P_b$ quantifies how accurate the predicted boundary is in terms of not including false positives. $R_b$ measures how completely the ground truth boundary is captured by the prediction.

\begin{equation}
P_b = \frac{|\partial A \cap \partial B_\varepsilon|}{|\partial B|}, \quad
R_b = \frac{|\partial A \cap \partial B_\varepsilon|}{|\partial A|}
\end{equation}

The General Index below combines the two measures by averaging them, providing an overall performance index.

\begin{equation}
\mathcal{G} = \frac{\mathcal{J} + \mathcal{F}}{2}
\end{equation}

 By using these three metrics in supplement to the overall validation set accuracy, a more robust performance display for the model can be shown. Since the given image sets in the DAVIS16 dataset have a significant amount of background pixels, which are easily separated from the foreground image in contrast to undesired objects with more prominence in the scene, the total accuracy may not be an accurate display of the model's performance. These three index metrics provide better insight into the actual segmentation and boundary accuracy capabilities of the guided slot attention model. 
...
\hline
\textbf{Method} & \textbf{Publication} & \textbf{Backbone} & \textbf{Resolution} & \textbf{FPS} & $\mathcal{G_M}$ & $\mathcal{J_M}$ & $\mathcal{F_M}$ \\ \hline
MATNet & AAAI'20 & ResNet101 & 473$\times$473 & 20.0 & 81.6 & 82.4 & 80.7 \\
WCS-Net & ECCV'20 & EfficientNetV2 & 320$\times$320 & 33.3 & 81.5 & 82.2 & 80.7 \\
F2Net & AAAI'21 & DeepLabV3 & 473$\times$473 & 10.0 & 83.7 & 83.1 & 84.4 \\
RTNet & CVPR'21 & ResNet101 & 384$\times$672 & - & 85.2 & 85.6 & 84.7 \\
FSNet & ICCV'21 & ResNet50 & 352$\times$352 & 12.5 & 83.3 & 83.4 & 83.1 \\
TransportNet & ICCV'21 & ResNet101 & 512$\times$512 & 12.5 & 84.8 & 84.5 & 85.0 \\
AMC-Net & ICCV'21 & ResNet101 & 384$\times$384 & 17.5 & 84.6 & 84.5 & 84.6 \\
HFAN & ECCV'22 & MiT-b2 & 512$\times$512 & 18.4 & 87.6 & 87.0 & 88.1 \\
HFAN* & ECCV'22 & MiT-b2 & 512$\times$512 & 2.9 & 88.7 & 88.0 & 89.3 \\
PMN & WACV'23 & VGG16 & 448$\times$256 & - & 85.9 & 85.4 & 86.4 \\
TMO & WACV'23 & ResNet101 & 384$\times$384 & \textbf{43.2} & 86.1 & 85.6 & 86.6 \\
OAST & ICCV'23 & MiT-b2 & 512$\times$512 & - & 87.0 & 86.6 & 87.4 \\ \hline
GSANet & CVPR'24 & ResNet101 & 512$\times$512 & 41.5 & 87.7 & 87.0 & 88.4 \\
GSANet* & CVPR'24 & MiT-b2 & 512$\times$512 & 4.1 & \textbf{88.9} & \textbf{88.3} & \textbf{89.6} \\
\textbf{Re-Implementation} &  & MiT-b2 & 480$\times$854 & - & \textbf{data} & \textbf{data} & \textbf{data} \\ \hline
\end{tabular}%
}
\label{tab:davis16}
\caption{Comparison of methods on DAVIS-16 dataset.}
\end{table*}
